...
export class TagService {
    ...
    async searchTags(userTags:Array<string>){
        ...
        .then((data)=>{
            let dataNameTag:Array<CreateTagData> = []
            data.forEach((element)=> dataNameTag.push(element.dataValues));
            userTags.forEach((element:string) => {
                if(dataNameTag.includes(element.dnameTag) === false){
    ...
    async checkForAvilabilityTags(userTags:Array<string>){
        ...
        .then(async (data)=> {
            console.log(data.length);(data.length !== 0)? await this.tagRepository.bulkCreate(data): undefined})
        .catch((error)=>`ERROR: ${error}`);
    }
}
