...
export class TagService {
    ...
    async searchTags(userTags:Array<CreateTagData>){
        ...
        .then((data)=>{
            ...
            userTags.forEach((element) => {
                console.log(dataNameTag.includes(element.nameTag))
                if(dataNameTag.includes(element.nameTag) !== true){
    ...
    async checkForAvilabilityTags(userTags:Array<CreateTagData>){
        ...
        .then(async (data)=> {
            if(data.length !== 0)? await this.tagRepository.bulkCreate(data): undefined;
