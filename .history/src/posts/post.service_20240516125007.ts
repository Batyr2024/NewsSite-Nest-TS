...
export class PostService {
    constructor(@InjectModel(Post) private postRepository: typeof Post,private tagRepository: typeof Tag,private tagService:TagService,private tagsPostsService:TagsPostsService) { }

    async createPost(postObject:CreatePostData,){
        try{
            const tag = this.tagService.checkForAvilabilityTags(postObject.tag)
            .then(async (data)=>{
                postObject.tag.forEach(async (element)=>{
