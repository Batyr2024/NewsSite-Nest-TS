...
export class PostService {
    ...
    async createPost(postObject:CreatePostData){
        try{
           postObject.tag =  await this.tagService.checkForAvilabilityTags(postObject.tag);
           const await this.postRepository.create(postObject,{ include: [{ model: Tag }] })
           //.then(async (data)=>{await this.postRepository.findByPk(data.id,{ include: [{ model: Tag }] })})
